// main.js — Two-row bonang layout with circular gong buttons.
const ROW_TOP = [
...
const topEl = document.getElementById('rowTop');
const bottomEl = document.getElementById('rowBottom');
...
ROW_TOP.forEach(n => topEl.appendChild(makePad(n)));
ROW_BOTTOM.forEach(n => bottomEl.appendChild(makePad(n)));
...
function initAudio(){
  ...
  unlocked = true;
  statusEl.textContent = 'Audio is ready — play with keys or tap the gongs.';
}
...
document.getElementById('frame').addEventListener('pointerdown', handleDown, {passive:false});
document.getElementById('frame').addEventListener('pointerup', handleUp);
document.getElementById('frame').addEventListener('pointerleave', handleUp);
